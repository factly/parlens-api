export function index({ db, logger }, { limit, page, q }) {
    ...
    return db.collection('politicalPartiesUnique').find(filter).sort({ PID: 1 }).limit(pageLimit).skip(pageSkip).toArray();
}
...
export function single({ db, logger }, { id }) {
    ...
    return db.collection('politicalPartiesUnique').findOne({ PID: id });
}