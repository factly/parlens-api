export async function index({ db, logger, config }, { limit, page, q }) {
    ...
    const nodes = await db.collection(config.db.constituencies).find(filter).sort({ CID: 1 }).skip(pageSkip).limit(pageLimit).toArray();
    
    const total = await db.collection(config.db.constituencies).find(filter).count()
